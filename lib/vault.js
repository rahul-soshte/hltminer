var INFURA_ROPSTEN_URL = 'https://ropsten.infura.io/v3/07b6050680014541860ede962a2489c8';
...
module.exports =  {
  ...
  async requirePassword(require)
  {
     ...
     if( requirePassword && userPassword == null ){
       ...
       if( !this.encryptedStorageExists() )
      {
        //build the storage folders if they (probably) dont exist anymore
          mkdirp(this.gethltLocalFolderPath(), function (err) {
            if (err) console.error(err)
            else console.log('created new keystore file at ' + this.gethltLocalFolderPath())
        }.bind(this));

      }

      localStorage = new LocalStorage(this.gethltLocalFolderPath()+'/hltminerstore');
    ...
    gethltLocalFolderPath()
    {
      return this.getOSLocalDataFolderPath()  + '/.hlt';
    },
   ...
   async handleAccountCommand(subsystem_command,subsystem_option)
    ...
    encryptedStorageExists()
    {
      ...
      if (!fs.existsSync(this.gethltLocalFolderPath()) || !fs.existsSync(this.gethltLocalFolderPath() +'/hltminerstore' )  ){
        return false;
      }

      if (!fs.existsSync(this.gethltLocalFolderPath()+'/hltminerstore/vaultdata' ) ){
        return false;
      }
