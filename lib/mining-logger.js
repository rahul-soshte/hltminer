...
module.exports =  {
  ...
  {
    this.vault = vault;
    logStorage = new LocalStorage(vault.gethltLocalFolderPath()+'/logs');
  ...
  logsExist()
  {
    if (!fs.existsSync(this.vault.gethltLocalFolderPath()) || !fs.existsSync(this.vault.gethltLocalFolderPath() +'/logs' )  ){
      return false;
    }

    if (!fs.existsSync(this.vault.gethltLocalFolderPath()+'/logs/stdout' ) ){
      return false;
    }
